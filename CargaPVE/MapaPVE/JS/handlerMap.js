class StaticMapHandler {
    ...
    exportSelectedPointsToCSV(selectedPoints) {
        // Ordenar los datos por el campo FECHA usando la función convertDateToISO
        const sortedPoints = selectedPoints.sort((a, b) => {
            const dateA = new Date(this.convertDateToISO(a.FECHA));
            const dateB = new Date(this.convertDateToISO(b.FECHA));
            return dateA - dateB;
        });

        // Convertir el array de objetos a CSV
        const csvContent = sortedPoints.map(point => {
            return Object.values(point).map(value => `"${value}"`).join(',');
        }).join('\n');

        // Agregar encabezados de columnas
        const headers = Object.keys(sortedPoints[0]).map(key => `"${key}"`).join(',');
    ...
    exportSelectedPointsToExcel(selectedPoints) {
        ...
        sortedPoints.forEach(point => { // Reemplazar comas por puntos en las coordenadas
            if (point.LATITUD) {
        ...
        const worksheet = XLSX.utils.json_to_sheet(sortedPoints); // Convertir los objetos a una hoja de calculo
    ...
    exportSelectedPointsToWord(selectedPoints) {
        ...
        const doc = new docx.Document({
            sections: [
                {
                    properties: {},
                    children: sortedPoints.flatMap(point => {
                        return [
                            new docx.Paragraph({
                                children: [
                                    new docx.TextRun({
