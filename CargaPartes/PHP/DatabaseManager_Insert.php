<?php
    ...
    class DataBaseManager_Insert {
        ...
        public function insertNewForm($Formulario, $Numero, $Año) {
            ...
            $stmt = null;
            try {
                $Division = $_SESSION['usergroup'];
                $usernameID = $_SESSION['usernameID'];
        
                $stmt = $this->conn->prepare("INSERT INTO entidad_encabezado (Formulario, Numero, Año, Division, UsuarioCreador) VALUES (?, ?, ?, ?, ?)");
        
                $types = $this->selectDataType([$Formulario, $Numero, $Año, $Division, $usernameID]);
        
                $stmt->bind_param($types, $Formulario, $Numero, $Año, $Division, $usernameID);
        ...
        // Método para actualizar el tipo de incidencia del Dispositivo SIACIP
        public function UPDATE_Estado($ID, $NuevoEstado) {
            // Iniciar la transacción
            $this->conn->begin_transaction();
            
            try {
                $stmt = $this->conn->prepare("UPDATE entidad_encabezado SET Estado = ? WHERE ID = ?");
                if (!$stmt) {
                    throw new Exception('Error al preparar la consulta: ' . $this->conn->error);
                }
        
                $stmt->bind_param("si", $NuevoEstado, $ID); // 'i' for integer type
                ...
                $stmt->close();
                $this->conn->commit();
                return true; // Cambiado para devolver true en caso de éxito
        
            } catch (Exception $e) {
                error_log($e->getMessage()); // Log the error message
                $this->conn->rollback();
                return false; // Cambiado para devolver false en caso de error
            }
        }      
        ...
        public function INSERT_Encabezado($form_data) {
            ...
            try {
                ...
                $Fiscal = $form_data['Fiscal'];
                $Relato = $form_data['Relato'];
                $usernameID = $_SESSION['usernameID'];

                // Comprobación para asignar $Clasificacion
                if ($_POST['Clasificacion'] === "Otra opción no listada") {
                    $Clasificacion = $_POST['OtraClasificacion'];
                } else {
                    $Clasificacion = $_POST['OtraClasificacion'];
                }

                $stmt = $this->conn->prepare("INSERT INTO entidad_encabezado (ID, Fecha, Hora, Clasificacion, Causa, Dependencia, Juzgado, Fiscal, Relato ,UsuarioCreador)
                                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                                    ON DUPLICATE KEY UPDATE
                                    Fecha=VALUES(Fecha), Hora=VALUES(Hora), Clasificacion=VALUES(Clasificacion), Causa=VALUES(Causa), Dependencia=VALUES(Dependencia), Juzgado=VALUES(Juzgado), Fiscal=VALUES(Fiscal), Relato=VALUES(Relato)");

                $types = $this->selectDataType([$ID, $Fecha, $Hora, $Clasificacion, $Causa, $Dependencia, $Juzgado, $Fiscal, $Relato, $usernameID]);

                $stmt->bind_param($types, $ID, $Fecha, $Hora, $Clasificacion, $Causa, $Dependencia, $Juzgado, $Fiscal, $Relato, $usernameID);
        ...
        public function INSERT_Persona($form_data) {
            ...
            try {
                ...
                $types = $this->selectDataType([$P_ClavePrimaria, $FK_Encabezado, $P_NumeroDeOrden, $P_FotoPersona, $P_Rol, $P_Apellido, $P_Nombre, $P_Alias, $P_Edad, $P_Genero, $P_DNI, $P_EstadoCivil, $P_Pais, $usernameID]);
                ...
                // Obtener el ID generado si $P_ClavePrimaria es nulo
                if (empty($P_ClavePrimaria)) {
                    $P_ClavePrimaria = $this->conn->insert_id;
                }
                ...
                while (isset($_POST["ID_Lugar$i"])) {
                    ...
                    $types = $this->selectDataType([$ID_Lugar, $P_ClavePrimaria, $L_NumeroDeOrden, $L_Rol, $L_Calle, $L_AlturaCatastral, $L_CalleDetalle, $L_Barrio, $L_Localidad, $L_Provincia, $L_Pais, $L_Coordenadas, $usernameID]);
                ...
                // Recopilar y procesar los datos de los datos complementarios
                $DC = 1; // Iniciar contador de instancias de los datos complementarios
                while (isset($_POST["ID_DatoComplementario$DC"])) {
                    // Recolectar datos del dato complementario actual
                    $ID_DatoComplementario = $_POST["ID_DatoComplementario$DC"];
                    $DC_NumeroDeOrden = $_POST["DC_NumeroDeOrden$DC"];
                
                    // Verificar si DC_NumeroDeOrden es igual a 0. Para este caso "0" significa sin asignar
                    ...
                    $types = $this->selectDataType([$ID_DatoComplementario, $P_ClavePrimaria, $DC_NumeroDeOrden, $DC_Tipo, $DC_ImagenAdjunta, $DC_Comentario, $usernameID]);
        ...
        public function INSERT_Lugar($form_data) {
            ...
            try {
                ...
                $types = $this->selectDataType([$L_ClavePrimaria, $FK_Encabezado, $L_NumeroDeOrden, $L_TipoLugar, $L_Rol, $L_NombreLugarEspecifico, $L_Calle, $L_AlturaCatastral, $L_CalleDetalle, $L_Interseccion1, $L_Interseccion2, $L_Barrio, $L_Localidad, $L_Provincia, $L_Pais, $L_Coordenadas, $usernameID]);
                ...
                // Obtener el ID generado si $L_ClavePrimaria es nulo
                if (empty($L_ClavePrimaria)) {
                    $L_ClavePrimaria = $this->conn->insert_id;
                }
                ...
                $stmt->close();
                    ...
                    while (isset($_POST["ID_DatoComplementario$DC"])) {
                        ...
                        $types = $this->selectDataType([$ID_DatoComplementario, $L_ClavePrimaria, $DC_NumeroDeOrden, $DC_Tipo, $DC_ImagenAdjunta, $DC_Comentario, $usernameID]);
        ...
        public function INSERT_Vehiculo($form_data) {
            ...
            try {
                ...
                $types = $this->selectDataType([$V_ClavePrimaria, $FK_Encabezado, $V_NumeroDeOrden, $V_TipoVehiculo, $V_Rol, $V_Color, $V_Marca, $V_Modelo, $V_Año, $V_Dominio, $V_NumeroChasis, $V_NumeroMotor, $usernameID]);
                ...
                // Obtener el ID generado si $L_ClavePrimaria es nulo
                if (empty($V_ClavePrimaria)) {
                    $V_ClavePrimaria = $this->conn->insert_id;
                }
                ...
                $stmt->close();
                    ...
                    while (isset($_POST["ID_DatoComplementario$DC"])) {
                        ...
                        $types = $this->selectDataType([$ID_DatoComplementario, $V_ClavePrimaria, $DC_NumeroDeOrden, $DC_Tipo, $DC_ImagenAdjunta, $DC_Comentario, $usernameID]);
        ...
        public function INSERT_Arma($form_data) {
            ...
            try {
                ...
                $types = $this->selectDataType([$AF_ClavePrimaria, $FK_Encabezado, $AF_NumeroDeOrden, $AF_EsDeFabricacionCasera, $AF_TipoAF, $AF_Marca, $AF_Modelo, $AF_Calibre, $AF_PoseeNumeracionVisible, $AF_NumeroDeSerie, $usernameID]);
                ...
                // Obtener el ID generado si $AF_ClavePrimaria es nulo
                if (empty($AF_ClavePrimaria)) {
                    $AF_ClavePrimaria = $this->conn->insert_id;
                }
                ...
                $stmt->close();
                    ...
                    while (isset($_POST["ID_DatoComplementario$DC"])) {
                        ...
                        $types = $this->selectDataType([$ID_DatoComplementario, $AF_ClavePrimaria, $DC_NumeroDeOrden, $DC_Tipo, $DC_ImagenAdjunta, $DC_Comentario, $usernameID]);
        ...
        public function INSERT_Secuestro($form_data) {
            // Iniciar la transacción
            $this->conn->begin_transaction();
        
            try {
                // Procesar los campos comunes desde $form_data
                $ClavePrimaria = $form_data['ClavePrimaria'];
                $FK_Encabezado = $form_data['ID'];
                $NumeroDeOrden = $form_data['NumeroDeOrden'];
                $DC_ImagenAdjunta = $form_data['Base64DC_ImagenAdjunta'];
                $DC_Comentario = $form_data['DC_Comentario'];
                $usernameID = $_SESSION['usernameID'];

                // Comprobación para asignar $DC_Tipo
                if ($_POST["DC_Tipo"] === "Otra opción no listada") {
                    $DC_Tipo = $_POST["DC_TipoEspecifique"];
                } else {
                    $DC_Tipo = $_POST["DC_Tipo"];
                }

                // Verificar si NumeroDeOrden es igual a 0
                if ($NumeroDeOrden == 0) {
                    $stmt = $this->conn->prepare("SELECT MAX(NumeroDeOrden) AS max_orden FROM entidad_datos_complementarios WHERE FK_Encabezado = ?");
                    $stmt->bind_param("s", $FK_Encabezado);
                    $stmt->execute();
                    $result = $stmt->get_result();
                    $row = $result->fetch_assoc();

                    // Determinar el nuevo valor de NumeroDeOrden
                    $NumeroDeOrden = $row['max_orden'] ? $row['max_orden'] + 1 : 1;

                    $stmt->close();
                }

                $stmt = $this->conn->prepare("INSERT INTO entidad_datos_complementarios (ID_DatoComplementario, FK_Encabezado, NumeroDeOrden, DC_ImagenAdjunta, DC_Comentario, DC_Tipo, DC_UsuarioCreador) 
                VALUES (?, ?, ?, ?, ?, ?, ?)
                ON DUPLICATE KEY UPDATE 
                NumeroDeOrden = VALUES(NumeroDeOrden), DC_ImagenAdjunta = VALUES(DC_ImagenAdjunta), DC_Comentario = VALUES(DC_Comentario), DC_Tipo = VALUES(DC_Tipo), DC_UsuarioCreador = VALUES(DC_UsuarioCreador)");

                $types = $this->selectDataType([$ClavePrimaria, $FK_Encabezado, $NumeroDeOrden, $DC_ImagenAdjunta, $DC_Comentario, $DC_Tipo, $usernameID]);

                $stmt->bind_param($types, $ClavePrimaria, $FK_Encabezado, $NumeroDeOrden, $DC_ImagenAdjunta, $DC_Comentario, $DC_Tipo, $usernameID);
